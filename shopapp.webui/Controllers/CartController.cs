...
namespace shopapp.webui.Controllers
{
    ...
    public class CartController : Controller
    {
        ...
        public IActionResult GetMiniCart()
        {
            ...
            if (!string.IsNullOrEmpty(userId))
            {
                ...
                var cartModel = new
                {
                    ...
                    items = cart.CartItems.Select(i => new
                    {
                        ...
                        ImageUrl = i.Product?.ImageUrl ?? "noimage.png",
            ...
            return Json(new { totalItems = 0, totalPrice = 0, items = new List<object>() });
        }
        ...
        [HttpPost]
        public IActionResult DeleteFromCart([FromBody] DeleteFromCartModel model)
        {
            try
            {
                var userId = _userManager.GetUserId(User);
                _cartService.DeleteFromCart(userId, model.ProductId);
                return Ok(); // Başarılı bir yanıt döndür
            }
            catch (Exception ex)
            {
                return BadRequest(new { message = ex.Message }); // Hata durumunda hata mesajı döndür
            }
        }
        ...
        {
            var userId = _userManager.GetUserId(User);
            _cartService.UpdateQuantity(userId, model.ProductId, model.Quantity);
            return Ok(); // Güncellemenin başarılı olduğunu belirtmek için HTTP 200 döner
        }
        ...
        private Payment PaymentProcess(OrderModel model)
        {
            ...
            options.ApiKey = "sandbox-Kdsw0w75JKbhCsuNs0gw02M6zNvLd2r5";
            options.SecretKey = "sandbox-mN0LCHbLqM0dcFxIPui1pngt3o2Xt53y";
